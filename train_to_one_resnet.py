...
from utils import synthesize_and_save_wavs, draw_melspectrograms, masked_mse_loss
...
def main():
    parser = argparse.ArgumentParser('PPG-VC trainer')
    ...
    for epoch in range(hps.TrainToOne.epochs):
        ...
        for idx, batch in enumerate(train_dataloader):
            # run forward pass
            optimizer.zero_grad()
            inputs = torch.cat([batch['ppg'], batch['log_f0']], dim=2).to(device)
        ...
        torch.save(model.state_dict(), os.path.join(args.model_dir, "ppg-vc-to-one-resnet-{}.pt".format(epoch)))
        # validation
        model.eval()
        dev_running_loss = 0.
        for dev_batch in dev_dataloader:
            dev_inputs = torch.cat([dev_batch['ppg'], dev_batch['log_f0']], dim=2).to(device)
        ...
        for test_batch in test_dataloader:
            test_inputs = torch.cat([test_batch['ppg'], test_batch['log_f0']], dim=2).to(device)
            test_outputs = model(test_inputs).transpose(0, 1)
            test_target_mels = test_batch['mel'].transpose(0, 1)
            synthesize_and_save_wavs(
